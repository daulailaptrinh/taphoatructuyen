 ...
 @section('content-ad')
     <div class="card shadow mb-4">
                         ...
                         <tbody style="text-align: center;">
                             @foreach ($sanpham1 as $key => $sp)
                                 <tr>
                                     ...
                                     <td>
                                         @php
                                             $gallery = App\Models\Gallery::where('gallery_product_id', $sp->id)
                                                 ->get()
                                                 ->count();
                                         @endphp
                                         @if ($gallery >= 5)
                                             <a class="btn btn-outline-success btn-sm"
                                                 href="{{ route('gallelyedit', $sp->id) }}">Gallery
                                                 ({{ $gallery }})</a>
                                         ...
                                         @else
                                             <a class="btn btn-outline-danger btn-sm"
                                                 href="{{ route('gallelyedit', $sp->id) }}">Ảnh chi tiết
                                                 ({{ $gallery }})</a>
                                         @endif
                                     </td>
                                     ...
                                     <td><img src="{{ asset('source/image/product/' . $sp->image) }}" alt=""
                                             width="100px"></td>

                                     <td>{{ isset($sp->id_type) ? $sp->product_type->name_type : '' }}</td>

                                     <td>
                                         <?php
                                   if($sp->new==1){
                                    ?>
                                         <a href="{{ url('/active-sp/' . $sp->id) }}"><span
                                                 class="far fa-thumbs-up"></span></a>
                                         <?php
                                     }else{
                                    ?>
                                         <a href="{{ url('/unactive-sp/' . $sp->id) }}"><span style="color: #e74a3b;"
                                                 class="far fa-thumbs-down"></span></a>
                                         <?php
                                   }
                                  ?>
                                     </td>
                                     <td style="width: 100px">
                                         <button class="btn btn-outline-primary edit" data-toggle="modal"
                                             data-id_pro="{{ $sp->id }}" type="button"><i
         ...
         <!-- Modal Add & Update -->
         <div class="modal fade" id="samepleModal" tabindex="-1" role="dialog" style="z-index: 1050; display: none;"
             aria-hidden="true">
                     ...
                     <form method="post" enctype="multipart/form-data" id="sampleSubmit">
                         ...
                         <div class="modal-body">
                             <div class="form-group">
                                 <label style="font-weight: bold; color: #000">{{ trans('Ql_sp.tensp') }} </label>
                                 <input type="text" id="sp_vi" name="sp_vi" class="form-control"
                             ...
                             <div class="form-group">
                                ...
                                <input type="number" id="quantity" name="quantity" class="form-control" readonly value="0" />
                            </div>
                            <div class="form-group">
                                <label style="font-weight: bold; color: #000">Giá nhập</label>
                                <input type="text" id="price_import" name="price_import" class="form-control" readonly value="0" />
                            </div>
                             <div class="form-group">
                             ...
                             <div class="form-group">
                                 ...
                                 <input type="number" id="e_promotion_price" name="promotion_price"
         ...
         <!-- Modal Del -->
         <div class="modal fade" id="showDel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
             aria-hidden="true">
                     ...
                     $.ajax({
                         ...
                         success: function(response) {
                             if (response.status == 200) {
                                 ...
                                 var price_import = document.getElementById('price_import');
                                 price_import.value = response.product.price_import;
                                 formatGia(price_import);
                                 ...
                                 $('#displayimg').html('<img src="source/image/product/' + response
                                     .product.image + '" alt="" width="200px">');
