...
<div class="card shadow mb-4">
    ...
    <div style="margin-top: 25px; margin-bottom: 1px; margin-left: 22px">
        <table>
            <tr>
                <button class="btn btn-outline-primary" data-toggle="modal" data-target="#couponAdd" type="button"
                    style="margin-right: 10px">
                    <i class="fa fa-plus" aria-hidden="true"></i> {{ trans('home_ad.add') }}
                </button>
            </tr>
            <tr>
                <a href="{{url('/send-coupon')}}" class="btn btn-outline-info"><i class="fas fa-mail-bulk"></i>
                    {{ trans('Ql_sp.guimagiamgia') }}</a>
            </tr>
            <tr>
                <button style="margin-left: 10px" type="button" class="btn btn-outline-success" data-toggle="modal"
                    data-target="#ExcelCoupon"><i class="fas fa-file-excel"></i>
            ...
            <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody style="text-align: center;">
                    ...
                    <tr>
                        ...
                        <td><?php $now9 = now()->format('d-m-Y'); ?>
                            @php
                                $now = strtotime(date('M j, Y'));
                                $end = strtotime(date("M j, Y",strtotime($cp->coupon_date_end)));
                            @endphp
                            @if($end >= $now) 
                        ...
                        <td style="width: 100px">
                            ...
                            <button class="btn btn-outline-primary edit" data-toggle="modal"
                                data-target="#couponUpdate_{{$cp->coupon_id}}" type="button"><i
                                    class="fas fa-pencil-alt"></i></button>
                            <!-- </a> -->
                            <!-- <a href="{{route('deletecoupon', $cp->coupon_id  )}}"> -->
                            <button class="btn btn-outline-danger delete" data-toggle="modal"
                                data-target="#couponDel_{{$cp->coupon_id}}" type="button"><i
                                    class="fas fa-trash-alt"></i></button>
                            <!-- </a> -->
                        </td>

                        <!-- Modal Delete-->
                        <div class="modal fade" id="couponDel_{{$cp->coupon_id}}" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button class="btn btn-secondary" type="button" data-dismiss="modal">Huỷ
                                            bỏ</button>
                        ...
                        <!-- Modal Update  -->
                        <div class="modal fade" id="couponUpdate_{{$cp->coupon_id}}" tabindex="-1" role="dialog"
                                    ...
                                    <form action="{{route('update_coupon', $cp->coupon_id)}}" method="post">
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group">
                                                <label
                                                    style="font-weight: bold; color: #000">{{ trans('Ql_sp.namecoupon') }}</label>
                                                <input type="text" id="coupon_name_{{$cp->coupon_id}}"
                                                    name="coupon_name" class="form-control"
                                                    value="{{$cp->coupon_name}}" />

                                            </div>
                                            <div class="form-group">
                                                <label
                                                    style="font-weight: bold; color: #000">{{ trans('Ql_sp.qtycoupon') }}</label>
                                                <input type="text" id="coupon_time_{{$cp->coupon_id}}"
                                                    name="coupon_time" class="form-control" multiple
                                                    value="{{$cp->coupon_qty}}" />
                                            </div>
                                            <div class="form-group">
                                                <label style="font-weight: bold; color: #000">Ngày Bắt Đầu</label>
                                                <input type="text" id="coupon_date_start_{{$cp->coupon_id}}"
                                                    name="coupon_date_start" class="form-control" multiple
                                                    value="{{$cp->coupon_date_start}}" />
                                            </div>
                                            <div class="form-group">
                                                <label style="font-weight: bold; color: #000">Ngày Kết Thúc</label>
                                                <input type="text" id="coupon_date_end_{{$cp->coupon_id}}"
                                                    name="coupon_date_end" class="form-control" multiple
                                                    value="{{$cp->coupon_date_end}}" />
                                            </div>

                                            <div class="form-group">
                                                <label
                                                    style="font-weight: bold; color: #000">{{ trans('Ql_sp.codecoupon') }}</label>
                                                <input type="text" id="coupon_code_{{$cp->coupon_id}}"
                                                    name="coupon_code" class="form-control" multiple
                                                    value="{{$cp->coupon_code}}" />
                                            </div>
                                            <div class="form-group" style="font-weight: bold; color: #000">
                                                <label
                                                    style="font-weight: bold; color: #000">{{ trans('Ql_sp.tinhnang') }}</label>
                                                <select name="coupon_condition" class="form-control">
                                                    <option <?php if($cp->coupon_condition==0)echo 'selected' ?>
                                                        value="0">{{ trans('Ql_sp.tinhnangphantram') }}</option>
                                                    <option <?php if($cp->coupon_condition==1)echo 'selected' ?>
                                                        value="1">{{ trans('Ql_sp.tinhnangtien') }}</option>
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <label
                                                    style="font-weight: bold; color: #000">{{ trans('Ql_sp.sophamtram') }}</label>
                                                <input type="text" id="coupon_number_{{$cp->coupon_id}}"
                                                    name="coupon_number" class="form-control" multiple
                                                    value="{{$cp->coupon_number}}" />
                                            </div>
                        ...
                        <tr>
                            <form action="{{url('/import-excel-coupon')}}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input" name="file" id="file" accept=".xlsx"
                                        required>
                                    <label class="custom-file-label" for="customFile">Choose file</label>
                                </div><br><br>
                                <button class="btn btn-outline-primary" type="submit" name="import_coupon"
                                    style="margin-right: 10px;">
                                    <i class="fas fa-file-import" aria-hidden="true"></i> {{ trans('home_ad.import') }}
                                    Excel
                                </button>
                            </form>
                        </tr>
                        <tr>
                            <form action="{{url('/export-excel-coupon')}}" method="POST">
                                ...
                                <button class="btn btn-outline-success" type="submit" name="export_coupon">
                                    <i class="fas fa-file-export" aria-hidden="true"></i> {{ trans('home_ad.export') }}
                                    Excel
                                </button>
                            </form>
                        </tr>
...
<!-- Modal Add-->
<div class="modal fade" id="couponAdd" tabindex="-1" role="dialog" style="z-index: 1050; display: none;"
    aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header text-write">
            ...
            <form action="{{route('addnewcoupon')}}" method="post">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label
                            style="font-weight: bold; color: #000">{{ trans('Ql_sp.namecoupon') }}</label>
                        <input type="text" id="coupon_name" name="coupon_name" class="form-control" multiple/>
                    </div>
                    <div class="form-group">
                        <label style="font-weight: bold; color: #000">{{ trans('Ql_sp.qtycoupon') }}</label>
                        <input type="number" min="1" id="coupon_time" name="coupon_time" class="form-control"
                            multiple />
                    </div>
                    <div class="form-group">
                        <label style="font-weight: bold; color: #000">Ngày Bắt Đầu</label>
                        <input type="date" onblur="chinhNgay(this, 'coupon_date_end')" id="coupon_date_start"
                            name="coupon_date_start" class="form-control" multiple />
                    </div>
                    <div class="form-group">
                        <label style="font-weight: bold; color: #000">Ngày Kết Thúc</label>
                        <input type="date" id="coupon_date_end" min="" name="coupon_date_end" class="form-control"
                            multiple />
                    </div>
